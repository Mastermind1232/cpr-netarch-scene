...
Shift+S (or the dish button on the token HUD) is the Scanner Meat Action. It runs the system's own Interface check (the CPR roll dialog with mods and LUCK, 3D dice, the CPR roll card) against DV 9 by default (stored per scene as `scanDv` in the NET flag, editable). The first Scan a character makes on a scene goes to chat, without the DV; later ones show only to the scanner. Jack In and Jack Out post a one-line notice. On a success the nearest hidden access point is revealed and marked as scanned, which makes it visible through walls from its own level only, so it never shows from inside the NET: Scanner gives the location, not a view. Needs libWrapper. The check must be made from the body, not the NET avatar.
