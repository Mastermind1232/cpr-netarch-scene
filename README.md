...
Shift+S (or the dish button on the token HUD) is the Scanner Meat Action. It runs the system's own Interface check (the CPR roll dialog with mods and LUCK, 3D dice, the CPR roll card) against DV 6 by default (stored per scene as `scanDv` in the NET flag, editable). Every Scan posts the roll card and a one-line outcome to chat, without the DV. Jack In and Jack Out post a one-line notice. On a success the nearest unfound access point is marked as found: a map pin appears on its spot. Pins ignore walls and fog, and carry an elevation range that Levels respects, so the pin shows on the floor and never from inside the NET. The access point token itself stays hidden. The GM can flip a point between found and not found with its eye toggle. The check must be made from the body, not the NET avatar.
